...
NEW_STORIES: list[StoryV2] = [
    ...
    StoryV2("S101", "cp-api: Pydantic Settings loading LOOP_CP_* env vars from .env + os.environ", "S2", "E12", 1, "P0", "AC: cp_api.config.Settings() loads LOOP_CP_DB_URL, LOOP_CP_REDIS_URL, LOOP_CP_AUTH_PROVIDER; missing-required raises at startup. [extends S019]"),
    StoryV2("S102", "cp-api: structlog wiring with request-id middleware + JSON output", "S2", "E12", 1, "P0", "AC: every request log line carries request_id, method, path, latency_ms, status; X-Request-Id header echoed."),
    ...
    StoryV2("S107", "cp-api: workspace-scope dependency injector (path-id → row + RBAC)", "S2", "E12", 2, "P0", "AC: any /v1/workspaces/{id}/* route auto-loads workspace + verifies caller is a member; non-member → 403."),
    StoryV2("S108", "cp-api: POST /v1/workspaces creates workspace + makes caller owner", "S2", "E12", 2, "P0", "AC: POST creates workspaces row, workspace_members(role='owner') row; returns 201 + WorkspaceResponse."),
    StoryV2("S109", "cp-api: GET /v1/workspaces lists workspaces caller belongs to", "S2", "E12", 1, "P0", "AC: returns paginated list filtered by caller's workspace_members."),
    StoryV2("S110", "cp-api: GET/PATCH /v1/workspaces/{id}", "S2", "E12", 1, "P0", "AC: GET returns workspace; PATCH updates name/region (owner-only); 403 for editor."),
    ...
    StoryV2("S112", "cp-api: GET/DELETE/PATCH /v1/workspaces/{id}/members/{user_id}", "S2", "E12", 2, "P0", "AC: list/remove/role-change; owner cannot demote last owner; integration tests for each path."),
    StoryV2("S113", "cp-api: POST /v1/workspaces/{id}/api-keys returns plaintext token once", "S2", "E12", 2, "P0", "AC: row stores prefix + bcrypt hash; response body is the only place the plaintext appears; 201 + ApiKeyCreated."),
    StoryV2("S114", "cp-api: GET /v1/workspaces/{id}/api-keys lists keys (no plaintext)", "S2", "E12", 1, "P0", "AC: returns id, prefix, created_at, scopes, revoked_at."),
    StoryV2("S115", "cp-api: DELETE /v1/workspaces/{id}/api-keys/{kid} revokes (soft delete)", "S2", "E12", 1, "P0", "AC: sets revoked_at; subsequent auth with that key → 401."),
    ...
    StoryV2("S118", "cp-api: error-mapper exception handler → LOOP-API-* coded JSON", "S2", "E12", 1, "P0", "AC: every framework/validation/auth exception emits {code, message, request_id} matching ERROR_CODES.md."),
    ...
    # =====================================================================
    StoryV2("S190", "kb: Document/Chunk/EmbeddingVector dataclasses with strict types", "S6", "E5", 1, "P0", "AC: pydantic models forbid extra; (de)serialize round-trip in unit tests. [extends S015]"),
    StoryV2("S191", "kb: Parser Protocol class + parser registry (ext → parser)", "S6", "E5", 1, "P0", "AC: register('.pdf', PdfParser) + lookup; unsupported ext raises UnsupportedDocumentType."),
    ...
    StoryV2("S194", "kb: text/MD parser with frontmatter detection", "S6", "E5", 1, "P0", "AC: .md with --- frontmatter parsed; metadata captured; body chunkable."),
    ...
    StoryV2("S196", "kb: chunker semantic_boundary (split on sentence + paragraph cohesion)", "S6", "E5", 3, "P0", "AC: target chunk_tokens=400±50; never splits mid-sentence; integration test on a 50-page sample doc."),
    StoryV2("S197", "kb: chunker fixed-size with overlap", "S6", "E5", 1, "P1", "AC: tokens=400, overlap=50; deterministic; unit-tested boundary cases (short doc, very long doc)."),
    StoryV2("S198", "kb: chunker by-heading (headings as natural splits)", "S6", "E5", 2, "P1", "AC: each top-level heading → chunk; sub-headings → context; tested on 3 fixtures."),
    ...
    StoryV2("S205", "kb: hybrid retrieval (RRF combine of vector + BM25)", "S6", "E5", 2, "P0", "AC: retrieve(query) returns RRF-ranked top_k; ablation test shows beating vector-only on a 50-q eval set."),
    ...
    StoryV2("S208", "kb: doc deletion with tombstone (chunks gone but provenance kept)", "S6", "E5", 1, "P1", "AC: deleted doc → chunks removed from Qdrant + tsvector; kb_documents row stays with deleted_at."),
    StoryV2("S209", "kb: re-ingest with content-hash diff (only changed chunks)", "S6", "E5", 2, "P1", "AC: re-uploading unchanged file → 0 new embeddings; partial change → only delta; metric emitted."),
    ...
    StoryV2("S221", "slack: signing-secret verification middleware (request body raw)", "S7", "E6", 2, "P0", "AC: tampered-signature requests → 401; valid → handler dispatch; unit-test fixtures (recorded webhooks)."),
    StoryV2("S222", "slack: /v1/slack/events handler + thread-aware conversation correlation", "S7", "E6", 2, "P0", "AC: thread_ts → conversation_id mapping table; integration: two messages in a thread → same conv."),
    StoryV2("S223", "slack: /v1/slack/commands slash-command handler", "S7", "E6", 1, "P0", "AC: /loop ask <q> dispatches to agent; immediate ack <3s; deferred response posted."),
    StoryV2("S224", "slack: Block Kit message renderer for AgentResponse", "S7", "E6", 2, "P0", "AC: text → blocks; lists → bulleted; code → mrkdwn-fenced; cites → context block."),
    ...
    StoryV2("S241", "eval: scorer exact_match (case-sensitive + insensitive variant)", "S8", "E8", 1, "P0", "AC: 1.0 on equal, 0.0 on not; unit tests."),
    StoryV2("S242", "eval: scorer rouge / bleu", "S8", "E8", 2, "P0", "AC: ROUGE-L f-score against reference; unit-tested on standard fixtures."),
    ...
    StoryV2("S244", "eval: scorer tool_call_match (right tool, right args, right order)", "S8", "E8", 2, "P0", "AC: passes when tools matched in order with arg-superset; unit tests cover misses."),
    StoryV2("S245", "eval: scorer latency_p50_under and cost_under_usd", "S8", "E8", 1, "P0", "AC: thresholds configurable; aggregates over case set; emit boolean + value."),
    StoryV2("S246", "eval: cassette format spec + recorder (gateway calls)", "S8", "E8", 2, "P0", "AC: cassette JSONL with request/response/usage; replayer round-trips identical."),
    StoryV2("S247", "eval: cassette replayer plugged into Gateway adapter", "S8", "E8", 2, "P0", "AC: replay-mode swaps real provider for cassette; unit tests verify deterministic outputs."),
    StoryV2("S248", "eval: case loader (YAML directory → list[EvalCase])", "S8", "E8", 1, "P0", "AC: tests/evals/*.yml load; schema validation errors named with file+line."),
    StoryV2("S249", "eval: parallel suite runner with concurrency cap + per-case timeout", "S8", "E8", 2, "P0", "AC: 100 cases run in <90s on dev laptop; one slow case doesn't block others."),
    StoryV2("S250", "eval: regression detector — compare vs baseline JSON", "S8", "E8", 2, "P0", "AC: detects mean-score drop ≥5% as regression; emits diff report."),
    StoryV2("S251", "eval: cli `loop eval run [suite]` with TAP-style output", "S8", "E11", 1, "P0", "AC: exit code 0 on pass, 1 on fail; --json emits machine-readable."),
    StoryV2("S252", "eval: production-replay capture middleware (sampling rate per agent)", "S8", "E8", 2, "P1", "AC: sampled turns persisted to NATS for later cassette write; opt-in flag."),
    ...
    # =====================================================================
    StoryV2("S260", "deploy: artifact bundler — package agent code + deps to zip", "S9", "E12", 2, "P0", "AC: bundler produces deterministic zip (same input → same sha256). [extends S020]"),
    ...
    StoryV2("S264", "deploy: k8s manifest renderer (Deployment + Service + HPA template)", "S9", "E12", 2, "P0", "AC: render({image, replicas, env}) yields valid YAML; kubeval green."),
    ...
    StoryV2("S268", "deploy: rollback API + automatic on canary failure", "S9", "E12", 2, "P0", "AC: POST /rollback or auto trigger reverts to previous_version; integration test."),
    ...
    StoryV2("S290", "obs: alert rules YAML — budget breach, error spike, latency p95", "S10", "E9", 2, "P0", "AC: rules.yaml file evaluated by alert-runner; alerts fire to webhook."),
    StoryV2("S291", "obs: alert delivery to Slack/email/PagerDuty (per workspace pref)", "S10", "E9", 2, "P0", "AC: per-workspace channel config; alert delivered with redacted PII."),
