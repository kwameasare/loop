...
class SamlSpConfig:
    ...
    """SAML attribute name that carries group memberships.

    Okta and Google Workspace use the plain ``groups`` attribute; Microsoft
    Entra ID uses the WS-Federation URI
    ``http://schemas.microsoft.com/ws/2008/06/identity/claims/groups``
    and emits group **Object IDs** rather than display names. The cp-api
    layer sets this field per IdP type when constructing the SP config.
    """
