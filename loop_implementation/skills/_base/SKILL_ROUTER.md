...
16. **Full implementation per story; combine when natural; never stub-ship.** Every story closed must have its acceptance criterion *independently* met by substantive code, with tests covering the happy path and ≥1 failure mode, and paired docs updated where the story touches their domain. Multiple bite-sized stories MAY be closed in a single PR or commit when they share a file, a skill, or one coherent diff — e.g. `feat(security): PASETO v4.local encode + decode (S105, S106)` — that's how combinable stories are *supposed* to flow. What's forbidden: (a) closing a story whose AC is not met, even if pytest is green against shallow scaffolding; (b) commit titles of the form `pass<N> close <K> stories` or `close <K> stories` — those are batch-grind tells, rename to describe substance; (c) closing ≥4 stories in a single commit (at that point the unit isn't natural — either the stories should be merged into one larger story, or you're stub-shipping). Enforced by `tools/check_no_bulk_close.py` (pattern guard, only flags `pass<N>`-style titles and ≥4-id commits) and `tools/audit_agent_behaviour.py` (substance check, periodic). When in doubt, ship one story at a time — extra commits are cheap.
17. **No direct commits to `main`.** All work goes through PR. Branch protection on GitHub MUST be applied per `docs/branch-protection.md` before any agent (parallel or solo) is allowed to commit. Enforced both by GitHub branch protection AND by a CI workflow that fails any push-event to `main` that wasn't a PR squash-merge.
18. **One open claim per agent.** When multiple agents are active, every agent holds at most ONE story in `In progress` / `Handing off` / `Blocked` at a time. Use `tools/pick_next_story.py` to select the next story. See `meta/parallel-work.md` for the full coordination protocol — claim races, hot-file etiquette, heartbeat cadence (≤30 min between checkpoints, not 4 h).
19. **`tools/agent_lifecycle.py` is the canonical interface.** Each agent self-manages its full lifecycle (`init` → `pick` → `claim` → `checkpoint`+ → `close` → `teardown`) through this one tool. Hand-rolled git/StoryV2/PR dances are forbidden — they're how the previous agent shipped 9 bulk-close commits. The tool refuses-loudly when state is wrong (double-claim, missing claim before checkpoint, dirty worktree on init/teardown, gates red on close). Each agent operates inside its own git worktree at `../bot-<owner>/` so `git checkout` in one is invisible to the others; `init` creates the worktree, `teardown` removes it. See `meta/AGENT_START_PROMPT.md` for the canonical sequence.
