...
export function KbRefreshPanel({
  agentId,
  documentId,
  opts = {},
}: KbRefreshPanelProps) {
  const [refreshStatus, setRefreshStatus] =
    React.useState<DocRefreshStatus | null>(null);
  ...
  if (loading) {
    return (
      <div
        data-testid="kb-refresh-loading"
  ...
  return (
    <div
      data-testid="kb-refresh-panel"
      className="space-y-2 rounded border bg-card p-3 text-sm"
    >
      {/* Status row */}
      ...
      <div className="text-muted-foreground">
        <span>Runs: </span>
        <span data-testid="kb-refresh-run-count">
          {refreshStatus?.runCount ?? 0}
        </span>
      ...
      {/* Error message */}
      {refreshStatus?.error && (
        <div data-testid="kb-refresh-error-msg" className="text-destructive">
          {refreshStatus.error}
        </div>
      )}
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {(
            Object.entries(REFRESH_CADENCE_LABELS) as [
              RefreshCadence,
              string,
            ][]
          ).map(([value, label]) => (
      ...
      {error && (
        <div
          data-testid="kb-refresh-api-error"
