...
vi.mock("@/lib/use-active-workspace", () => ({
  useActiveWorkspace: () => ({
    active: { id: "ws_costs", name: "Costs Workspace" },
    isLoading: false,
  }),
}));
...
describe("CostsPage", () => {
  ...
  it("shows degraded cost evidence instead of a raw route alert when cp-api is unavailable", async () => {
    ...
    await waitFor(() => {
      ...
      expect(state).toHaveTextContent(/Cost evidence is degraded/i);
      expect(state).toHaveTextContent(/usage and cost evidence/i);
