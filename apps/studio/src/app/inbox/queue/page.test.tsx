...
describe("InboxQueuePage", () => {
  ...
  it("opens queue links with the requested agent filter already applied", async () => {
    process.env.LOOP_CP_API_BASE_URL = "https://cp.test/v1";
    ...
    await waitFor(() => {
      expect(view.getByTestId("queue-focused-agent")).toHaveTextContent(
        "agent_billing",
      );
      expect(view.getByTestId("queue-filter-agent")).toHaveValue(
        "agent_billing",
      );
    });
    expect(view.getByTestId("queue-row-item_billing")).toBeInTheDocument();
    expect(view.queryByTestId("queue-row-item_support")).toBeNull();
  });
});
